...
def rabin_karp(text, pattern, q=101):
    ...
    for s in range(n - m + 1):
        comparisons += 1  # Hash comparison check
        if p_hash == t_hash:
            # Spurious hit check / Exact match validation
            match = True
            for k in range(m):
        ...
        if s < n - m:
            # Slide the window, compute next hash
...
# Performance comparison on a larger dataset
